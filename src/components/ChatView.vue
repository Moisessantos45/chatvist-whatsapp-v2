<template>
  <div :class="[
    ...
    <div v-if="!cluster.clave" class="flex-1 flex items-center justify-center bg-[#0f1014]">
      <div class="text-center">
        <MessageCircle class="w-20 h-20 text-gray-700 mx-auto mb-5 stroke-[1.5]" />
        <h2 class="text-lg font-medium text-gray-300 mb-2">WhatsApp Clone</h2>
    ...
    <div v-else class="flex flex-col h-full bg-[#13151a]">
      <!-- Header del chat -->
      <div class="bg-[#1a1d24] px-5 py-3 border-b border-gray-800 flex items-center">
        <!-- Botón de regreso (solo móvil) -->
        <button v-if="isMobile" @click="goBack" class="mr-3 p-2 -ml-2 rounded-md hover:bg-white/10 transition-colors">
          <ArrowLeft class="w-5 h-5 text-gray-400 hover:text-white" />
        </button>
        ...
        <div class="flex items-center flex-1 cursor-pointer">
          ...
          <div>
            <h3 class="font-semibold text-gray-100 leading-tight">{{ cluster.nombre }}</h3>
          </div>
        </div>

        <!-- Acciones del chat -->
        <div class="flex items-center space-x-1">
          <button class="p-2 rounded-md hover:bg-white/10 transition-colors text-gray-400 hover:text-white">
            <Search class="w-5 h-5" />
          </button>
          <button @click="showChatOptionsModal = true"
            class="p-2 rounded-md hover:bg-white/10 transition-colors text-gray-400 hover:text-white">
      ...
      <!-- Área de mensajes -->
      <div class="flex-1 relative flex flex-col overflow-hidden bg-[#0b0c0f] chat-background-dark">
        ...
        <div ref="messagesContainer" @scroll="handleScroll" class="flex-1 overflow-y-auto px-5 py-6 z-10">
          <div class="space-y-3">
            <div v-for="message in messages" :key="message.id" :class="[
              ...
              <div :class="[
                ...
                isReplyingToMessage(message.id) ? 'ring-2 ring-whatsapp-dark-blue/50 ring-offset-2 ring-offset-[#0b0c0f]' : ''
              ]">
                ...
                <div v-if="message.usuarioId !== user.id" class="text-[13px] font-semibold text-whatsapp-light mb-1">
                ...
                <div class="absolute bottom-1 right-2 flex items-center">
                  <span
                    :class="['text-[11px] font-medium', message.usuarioId === user.id ? 'text-blue-100/70' : 'text-gray-500']">
                    {{ formaterDate(message.fecha) }}
                  </span>
                  <Check v-if="message.usuarioId === user.id" :class="[
                    'w-3.5 h-3.5 ml-1',
                    true ? 'text-blue-200' : 'text-blue-100/50'
                  ]" />
                </div>
              </div>
            </div>
          ...
          <button v-if="showScrollButton" @click="scrollToBottom"
            class="absolute bottom-4 right-5 z-20 p-2.5 rounded-full bg-[#1e2128] shadow-[0_4px_14px_0_rgb(0,0,0,0.5)] border border-gray-700 text-gray-300 hover:bg-white/10 transition-colors flex items-center justify-center group"
            aria-label="Ir al último mensaje">
            <ChevronDown class="w-5 h-5 text-gray-300" />
        ...
        <div class="bg-[#13151a] px-4 py-3 border-t border-gray-800">
          ...
          <div v-if="replyingToMessage"
            ...
            <button @click="cancelReply"
              class="absolute top-2 right-2 p-1 rounded-md hover:bg-white/10 transition-colors">
          ...
          <div class="flex items-end space-x-2">
            <button
              class="p-2 sm:p-3 rounded-full hover:bg-white/5 transition-colors mb-0.5 text-gray-400 hover:text-gray-300">
            ...
            <div class="flex-1 relative">
              ...
              <div v-if="showMentionList && filteredUsers.length > 0"
                class="absolute bottom-full left-0 right-0 mb-2 bg-[#1a1d24] border border-gray-800 rounded-xl shadow-lg max-h-48 overflow-y-auto z-10 py-1">
                <div v-for="(user, index) in filteredUsers" :key="user.id" @click="selectMention(user)" :class="[
                  ...
                  index === selectedMentionIndex ? 'bg-white/10' : 'hover:bg-white/5'
                ]">
                  <div
                  ...
                  <span class="text-sm font-medium text-gray-100">{{ user.nombre }}</span>
                </div>
              </div>
              ...
              <textarea ref="textareaRef" v-model="message.contenido"
                ...
                class="w-full px-4 py-2.5 bg-[#1e2128] rounded-lg border border-transparent focus:border-transparent focus:ring-0 transition-colors resize-none overflow-y-auto text-gray-100 placeholder-gray-500"
